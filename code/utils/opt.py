...
def merge_dataset_stats(train, val, test):
    train_id, train_g, train_e, train_e_a = train._get_pool()
    test_id, test_g, test_e, test_e_a = test._get_pool()
    val_id, val_g, val_e, val_e_a = val._get_pool()
    m_ids, m_gs, e_s = [], [], []
    max_latent_nodes = max([train.max_latent_nodes, 
                                 test.max_latent_nodes, 
                                 val.max_latent_nodes])
    e_as = Merge(Merge(train_e_a, test_e_a), val_e_a)
    ...
    return m_ids, m_gs, e_s, e_as, max_latent_nodes