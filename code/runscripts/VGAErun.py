...
parser.add_argument(
    "-save_graph_structure_dir", type=str, default="../logs/graph_structure/"
)
...
def main():
    ...
    args.graph_structure_aelayers_dir = os.path.join(
        args.save_graph_structure_dir, f"{args.instance_id}", f"{args.ae_layers}"
    )
    # this attribute is also used in encoder ^
    if not os.path.isdir(args.graph_structure_aelayers_dir):
        save_bi_stride_dir(args, dataset)

    m_ids, m_gs, e_s = get_bi_stride_structure(args)
